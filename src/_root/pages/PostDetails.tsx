    ...
    <div className="post_details-container">
      {isPending ? 'Loading...' : (
        <div className="post_details-card">
          <img src={post?.imageUrl || '/assets/icons/profile-placeholder.svg'} alt="creator" className="post_details-img" />
            ...
            <div className="flex-between w-full">
              <Link to={`/profile/${post?.creator.$id}`} className="flex items-center gap-3">
                ...
                <div className="flex flex-col ">
                  <p className="base-medium lg:body-bold text-dark-4 capitalize">
                    {post?.creator.name}
                  </p>
                  <div className="flex-center gap-2 text-indigo-900">
            ...
            <div className="flex flex-col flex-1 w-full small-medium lg:base-regular">
              ...
              <ul className="flex gap-1 mt-2">
                {post?.tags.map((tag: string) => (
                  <li key={tag} className="text-indigo-600">#{tag}</li>
                ))}
              </ul>
            </div>
